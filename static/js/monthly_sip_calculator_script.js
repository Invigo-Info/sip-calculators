...
function setupEventListeners() {
    ...
    const inputs = [
        { input: 'sipAmount', slider: 'sipAmountSlider', min: 500, max: 100000 },
...
function calculateAndUpdate() {
    ...
    // Make API call to Monthly SIP calculation endpoint
    fetch('/calculate-monthly-sip', {
        method: 'POST',
        headers: {
...
function downloadPDF() {
    try {
        ...
        doc.text('Monthly SIP Calculator Results', 20, 30);
        ...
        doc.setFontSize(11);
        doc.setFont('helvetica', 'normal');
        doc.text(`Monthly SIP Amount: ₹${sipAmount.toLocaleString()}`, 20, 75);
        ...
        doc.save('monthly-sip-calculation.pdf');
...
function downloadExcel() {
    try {
        ...
        csvContent += "Monthly SIP Calculator Results\n\n";
        csvContent += "Input Parameters:\n";
        csvContent += `Monthly SIP Amount,${sipAmount.toLocaleString()}\n`;
        ...
        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
            // For IE
            window.navigator.msSaveOrOpenBlob(blob, 'monthly-sip-calculation.csv');
        } else {
            // For other browsers
            const url = URL.createObjectURL(blob);
            link.setAttribute('href', url);
            link.setAttribute('download', 'monthly-sip-calculation.csv');
...
function generatePrintableContent() {
    ...
    let content = `Monthly SIP Calculator Results\n`;
    content += `Generated on: ${new Date().toLocaleDateString()}\n\n`;
    
    content += `INPUT PARAMETERS:\n`;
    content += `Monthly SIP Amount: ₹${sipAmount.toLocaleString()}\n`;
...
function shareLink() {
    try {
        ...
        if (navigator.share && /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
            navigator.share({
                title: 'Monthly SIP Calculator Results',
                text: `Check out my Monthly SIP calculation: ₹${sipAmount} monthly for ${tenureYears} years at ${returnRate}% return rate`,
                url: shareUrl,
            }).then(() => {
