...
// Custom Chart.js plugin to display CAGR in center
const centerTextPlugin = {
    ...
    beforeDraw: function(chart) {
        if (chart.config.options.plugins.centerText && chart.config.options.plugins.centerText.display) {
            ...
            ctx.fillText('CAGR', centerX, centerY + 15);
...
function updateResultsDisplay(result) {
    ...
    document.getElementById('cagrResult').textContent = `${result.cagr_percentage.toFixed(2)}%`;
...
function updateChart(result) {
    const ctx = document.getElementById('cagrBreakupChart').getContext('2d');
    ...
    chart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                centerText: {
                    display: true,
                    text: `${result.cagr_percentage.toFixed(2)}%`
                }
            },
            animation: {
                animateScale: true,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (megaMenuBtn && megaMenuContent) {
        megaMenuBtn.addEventListener('click', function(e) {
            e.preventDefault();
            const isOpen = megaMenuContent.style.display === 'block';
            megaMenuContent.style.display = isOpen ? 'none' : 'block';
        });
